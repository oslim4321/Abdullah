...
const Attachment = ({ textArea, setTextArea, upload, setUpload }) => {
    const [fileList, setFileList] = useState([]);
    const [uploading, setUploading] = useState(false); // Track uploading state

    useEffect(() => {
        // Load initial images from upload into the fileList
        setFileList(
            upload.map((item) => ({
                uid: item.id,
                name: item.url,
    ...
    const handleRemove = async (file) => {
        try {
            ...
            const updated = upload.filter((item) => item.url !== file.url);
            setUpload(updated);
    ...
    return (
        <div className='p-10 w-full mt-20 h-[340px] gap-5 grid md:grid-cols-2 grid-cols-1'>
            <Dragger
            ...
            <TextArea placeholder='Message to Seller' rows={4} onChange={(e) => setTextArea(e.target.value)} value={textArea} />
        </div>
    );
}
