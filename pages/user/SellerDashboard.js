...
import { Menu, Dropdown, Spin, message } from "antd";
import {
  MenuOutlined,

} from "@ant-design/icons";
import { getLoggedInUserEnquries, getUserCategoriesSpecificEnquiries } from "../../lib/enquiry";
...
const SellerDashboard = () => {
  ...
  function fetchEnquires() {
    getUserCategoriesSpecificEnquiries(user?.userId)
      .then(enq => { setEnquires(enq); setIsEnquiryLoading(false); })
  ...
  const renderSelectedInquiryComments = () => {
    if (selectedInquiryIndex !== null) {
      ...
      return <CommentsList fetchEnquires={fetchEnquires} documentId={selectedInquiry.id} hide={true}
        comments={
    ...
    return (
      <div className=' w-full '>
        <Spin spinning={isEnquiryLoading}>
          <h1 className="text-center mb-2 text-lg font-bold">Enquiries According to your categories</h1>
          {
            enquries?.map((enquiry, index) => (
              <>
                <div key={index}
                  ...
                  <div className="flex w-full justify-end space-x-2 items-center mt-3 smd:mt-0 mb-2">
                    ...
                    <Link href={`/inquiry/${enquiry?.enquiryId}`}>View and Quote</Link>
                  </div>
                </div>
                {
